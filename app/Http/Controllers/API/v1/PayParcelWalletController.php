...
class PayParcelWalletController extends Controller
{
    ...
    public function UpdatePayRequeteWallet(Request $request)
    {
        ...
        if ($updatedata > 0) {
            ...
            // Get user info
            $query = ParcelOrder::crossJoin('tj_user_app')
            ...
            // Get Ride Info
            $ride = ParcelOrder::select('distance', 'distance_unit','duration','discount', 'tax','parcel_image','created_at')->where('id', '=', $id_requete)->first();

            $distance = $ride->distance;
            $distance_unit = $ride->distance_unit;
            $date_heure = $ride->created_at;
            $duree = $ride->duration;
