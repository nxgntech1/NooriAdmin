        ...
        <div class="row page-titles">
            ...
            <div class="col-md-7 align-self-center">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">{{trans('lang.booking_types')}}</li>
                </ol>
            </div>
            <div>
            </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="userlist-topsearch d-flex mb-3">
                                <div class="userlist-top-left">
                                    <a class="nav-link do_not_create" href="{!! route('bookingtypes.create') !!}"><i
                                                class="fa fa-plus mr-2"></i>{{trans('lang.create_bookingtype')}}</a>
                                </div>
                                <div id="users-table_filter" class="ml-auto">
                                    <label>{{ trans('lang.search_by')}}
                                    <div class="form-group mb-0">
                                        <form action="{{ route('bookingtypes') }}" method="get">
                                            ...
                                            <div class="search-box position-relative">
                                                ...
                                                <a class="btn btn-warning btn-flat" href="{{url('bookingtypes')}}">Clear</a>
                                            </div>
                                        </form>
                                    </div>
                                    </label>
                            ...
                            <div class="table-responsive m-t-10">
                                <table id="example24"
                                    ...
                                    @if(count($bookingtypes) > 0)
                                    @foreach($bookingtypes as $value)
                                        <tr>
                                            ...
                                            <td><a href="{{route('bookingtypes.edit', ['id' => $value->id])}}">{{ $value->bookingtype}}</a> </td>
                                            ...
                                            <td class="action-btn">
                                            <a
                                                        href="{{route('bookingtypes.edit', ['id' => $value->id])}}" class="do_not_edit"><i
                                                            class="fa fa-edit"></i></a><a id="'+val.id+'"
                                                                                          class="do_not_delete"
                                                                                          name="user-delete"
                                                                                          href="{{route('bookingtypes.delete', ['id' => $value->id])}}"><i
                                ...
                                <nav aria-label="Page navigation example" class="custom-pagination">
                                    {{ $bookingtypes->appends(request()->query())->links()  }}
                                </nav>
                                {{ $bookingtypes->links('pagination.pagination') }}
                            </div>
                        </div>
                    </div>
                </div>
        ...
        $("#deleteAll").click(function () {
            if ($('#example24 .is_open:checked').length) {
                if (confirm('Are You Sure want to Delete Selected Data ?')) {
                    ...
                    var url = "{{url('bookingtypes/delete', 'id')}}";
