                        ...
                        <div class="card-body">
                            ...
                            <div class="userlist-topsearch d-flex mb-3">
                                <div class="userlist-top-left">
                                    <a class="nav-link do_not_create" href="{!! route('cmpricing.create') !!}"><i
                                                class="fa fa-plus mr-2"></i>{{trans('lang.create_model_price')}}</a>
                                </div>
                                <div id="users-table_filter" class="ml-auto">
                                    <label>{{ trans('lang.search_by')}}
                                    <div class="form-group mb-0">
                                        <form action="{{ route('cmpricing') }}" method="get">
                                            @if(isset($_GET['selected_search']) &&  $_GET['selected_search'] != '')
                                                ...
                                                <select name="selected_search" id="selected_search"
                                                    ...
                                                    <option value="bookingtype" @if ($_GET['selected_search']=='bookingtype')
                                                    @endif >{{ trans('lang.bookingtype_name')}}</option>
                                                </select>
                                            @else
                                                <select name="selected_search" id="selected_search"
                                                        class="form-control input-sm">
                                                    ...
                                                    <option value="bookingtype">{{ trans('lang.bookingtype_name')}}</option>
                                                </select>
                                            @endif
                                            <div class="search-box position-relative">
                                                ...
                                                <a class="btn btn-warning btn-flat" href="{{url('cmpricing')}}">Clear</a>
                                            </div>
                                        </form>
                                    </div>
                                    </label>
                                ...
                                <table id="example24"
                                    ...
                                    @foreach($carModelPricing as $value)
                                        <tr>
                                            ...
                                            <td class="action-btn">
                                            <a
                                                        href="{{route('cmpricing.edit', ['id' => $value->PricingID])}}" class="do_not_edit"><i
                                                            class="fa fa-edit"></i></a><a id="'+val.PricingID+'"
                                                                                          class="do_not_delete"
                                                                                          name="user-delete"
                                                                                          href="{{route('cmpricing.delete', ['id' => $value->PricingID])}}"><i
        ...
        $("#deleteAll").click(function () {
            if ($('#example24 .is_open:checked').length) {
                if (confirm('Are You Sure want to Delete Selected Data ?')) {
                    ...
                    var url = "{{url('cmpricing/delete', 'id')}}";
